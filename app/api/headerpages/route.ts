...
export async function GET(request: NextRequest) {
  try {
    ...
    const headerHome = await prisma.headerPages.findMany();
    return NextResponse.json(headerHome, { status: 200 });
  } catch (error) {
...
export async function POST(request: NextRequest) {
    ...
    const newHeaderHome = await prisma.headerPages.create({
      data: {
        title,
        opacity,
        image: imageUrl,
    ...
    return NextResponse.json(newHeaderHome, { status: 201 });
}