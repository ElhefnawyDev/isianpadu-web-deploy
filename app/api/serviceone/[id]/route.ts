...
export async function PATCH(request: NextRequest, {params}: {params:{id:string,}}){
    ...
    if(!ServicesOne) return NextResponse.json({error:'Invalid Issue'}, {status: 404});
      ...
      const saveFile = async (file: File): Promise<string> => {
        ...
        const updatedServicesOne = await prisma.servicesOne.update({
            where: {id: ServicesOne.id},
            data: {
                title,
                description,
                icon: iconeUrl,
        ...
        return NextResponse.json(updatedServicesOne);
    }
    ...
    export async function DELETE(request: NextRequest, {params}: {params:{id:string,}}){ 
            ...
            await prisma.servicesOne.delete({
                where: {id: ServicesOne.id}
            })
