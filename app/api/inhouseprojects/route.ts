...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(inHouseProjects, { status: 200 });
  } catch (error) {
...
export async function POST(request: NextRequest) {
    ...
    const newInHouseProjects = await prisma.inHouseProjects.create({
      data: {
        title,
        description,
        ldescription,
    ...
    return NextResponse.json(newInHouseProjects, { status: 201 });
}